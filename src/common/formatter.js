...
export function toHex(mixed) {
  ...
  if (typeof mixed === 'string') {
    ...
    return regex.test(mixed) ? mixed : '0x' + ethUtil.toBuffer(+mixed).toString('hex');
  }
...
export function toBaseUnitAmount(number, digits) {
  return toBig(number).times('1e' + digits)
}

export function toUnitAmount(number, digits) {
  if (!number) {
    return number
  }
...
export function fromNumberToPrice(hex, digits) {
  let amount = toUnitAmount(toNumber(hex), digits);
  return amount
}

export function formatDateTime(timestamp, formatter) {
